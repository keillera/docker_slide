...
## 　
## ついでに chat ルームも作ってみました
## ※ インターネットに公開されてるので業務情報等は絶対に書き込まないでください
...
## マサカリ投げる場合（指摘、ツッコミする）は私がへこまない程度にお願いします
...
## すでにGoogleは全てのサービスをコンテナで運用しており毎週20億個ものコンテナを起動しているらしいです。
## 　
## ※ Dockerとは言っていない

---

## 実績もあるのでコンテナ化は今後もっと使われそうなイメージです。
...
## 繰り返しますが kernel は 3.8 以上である必要があります。よく業務で使われてる CentOS を調べてみると・・・
...
## ローカル環境で Docker を動かしたい場合は boot2docker といった Docker 用の軽量なディストリビューションを利用することをおすすめします。VirtualBox 上で動かします（後ほどデモで利用します）。
...
## その他のOSについてはごめんなさい、ここ見てください
- https://docs.docker.com/installation/#installation

## 各プラットフォームに対して丁寧に解説がされています（ムービー付きもあるよ！）
...
# docker build -t keiju/nginx .
# 作成したDocker Image を確認$ docker images
```
...
```sh
# Docker Image をコンテナとして起動
# docker run -d -p ホスト:コンテナ --name コンテナ名 image
$ docker run -d -p 8080:80 --name nginx_test keiju/nginx
# boot2docker を使っており、VirtualBoxのipを知りたい場合
$ boot2docker ip
```
...
## ただ、ファイルのアップロードやダウンロードに時間がかかるので
## 3分間クッキング方式でやらせて頂きます
...
## このため、アプリのログデータやDBデータ等の動的に作成されるデータを保存する場合は、何かしら対策しておく必要があります
...
- 環境ごとの差分を意識しなくてよい
- クリーンな環境で作業ができる
- あらかじめ想定している環境でテストできるため、デプロイ自体も安全に行える

---

##  Docker では設定を変更するときは Docker Image を作り直し新たに環境を作成して、古い環境は捨てる
## ↓
...
- Docker ならではの使いにくさがある（例：起動するときにプライベートIPを設定できないためコンテナ間での通信の際にDNS等を挟む等の対応が必要）
- 複数のコンテナを管理するツールがまだ未成熟

---

- Linux のプロセスを分けて仮想化しているため、Linux 以外のOSを仮想化させて動かす事ができない。このためLinux kernel を使用していない Windows は Docker 上で利用することはできません
ただ、先月(2014/10) 時期Windows Server で Docker を採用するとの発表がありました。具体的な実装方法等は公開されていませんが Windows アプリを利用しているシステムでも Docker を利用できるようになるかもしれません。
...
※ ホストOS の kernelバージョンによる差異は発生する
- 有名どころの会社がどんどんDocker関係で集まってきている
...
## まとめ
## 　
## まだ本番環境として使えるものではなさそうです。現状としてはシンプルなシステムに対して開発環境で使う程度が良いかと思います

---

# 今後くるであろう Docker の流れに備えておきましょう
